...
function init() {
  ...
  if (!canvas) return;
  if (canvas.dataset.solutionInitialized === 'true') return;
  canvas.dataset.solutionInitialized = 'true';
  ...
  const classical = document.getElementById('classical');  // NEW
...
function draw(t, hat_u) {
  ...
  const xs = new Array(N), ySum = new Array(N), yR = new Array(N), yLtrav = new Array(N), ySumC = new Array(N), yRC = new Array(N), yLC = new Array(N);
  ...
  for (let i = 0; i < N; i++) {
    ...
    uSum = hat_u[0] * Math.sin(theta) * Math.cos(omega(1) * t) / 2;
    uSum += hat_u[1] * Math.sin(k * theta) * Math.cos(omega(k) * t) / 2;

    uRight = hat_u[0] * 0.5 * Math.sin(theta - omega(1) * t);
    uRight += hat_u[1] * 0.5 * Math.sin(k * theta - omega(k) * t);

    uLeft = hat_u[0] * 0.5 * Math.sin(theta + omega(1) * t);
    uLeft += hat_u[1] * 0.5 * Math.sin(k * theta + omega(k) * t);

    ySum[i]  = uSum; yR[i]    = uRight; yLtrav[i]= uLeft;

    let uSumC = 0, uRightC = 0, uLeftC = 0; // NEW

    // classical counterparts: ω(k)=c*k
    uSumC  = hat_u[0] * Math.sin(theta)      * Math.cos((c * 1) * t) / 2
        + hat_u[1] * Math.sin(k * theta)  * Math.cos((c * k) * t) / 2;

    uRightC = hat_u[0] * 0.5 * Math.sin(theta      - (c * 1) * t)
            + hat_u[1] * 0.5 * Math.sin(k * theta  - (c * k) * t);

    uLeftC  = hat_u[0] * 0.5 * Math.sin(theta      + (c * 1) * t)
            + hat_u[1] * 0.5 * Math.sin(k * theta  + (c * k) * t);

    ySumC[i] = uSumC;  yRC[i] = uRightC;  yLC[i] = uLeftC; // NEW
  ...
  if (classical && classical.checked) {
    ctx.save();
    ctx.setLineDash([6,4]);       // dashed overlay
    ctx.lineWidth = 2;
    ctx.strokeStyle = '#A81C3A';  
    // Left panel classical
    ctx.beginPath();
    for (let i = 0; i < N; i++) {
        const xp = xL(xs[i]), yp = yL(ySumC[i]);
        if (i === 0) ctx.moveTo(xp, yp); else ctx.lineTo(xp, yp);
    }
    ctx.stroke();

    ctx.strokeStyle = '#a35868';

    // Top-right classical
    ctx.beginPath();
    for (let i = 0; i < N; i++) {
        const xp = xRT(xs[i]), yp = yRT(yRC[i]);
        if (i === 0) ctx.moveTo(xp, yp); else ctx.lineTo(xp, yp);
    }
    ctx.stroke();
    ...
    ctx.beginPath();
    for (let i = 0; i < N; i++) {
        const xp = xRB(xs[i]), yp = yRB(yLC[i]);
        if (i === 0) ctx.moveTo(xp, yp); else ctx.lineTo(xp, yp);
    }
    ctx.stroke();
...
//   ctx.fillText('x', rightB.x+ rightB.w - 10, rightB.y+ rightB.h + 22);
  ...
  function update() {
    ...
    const hat_u = [
        parseFloat(u1 ? u1.value : 1),
        parseFloat(u2 ? u2.value : 0),
    ];

    if (tVal) tVal.textContent = t.toFixed(2);   
    if (u1Val) u1Val.textContent = hat_u[0].toFixed(2);
    if (u2Val) u2Val.textContent = hat_u[1].toFixed(2);
...
[u1, u2, time, classical].filter(Boolean).forEach(el => el.addEventListener('input', update)); // NEW
