(function () {
function init() {
  const canvas = document.getElementById('waveDiscontinuousCanvas');
  if (!canvas) return;
  if (canvas.dataset.waveDiscontinuousInitialized === 'true') return;
  canvas.dataset.waveDiscontinuousInitialized = 'true';
  const ctx = canvas.getContext('2d');
  const w = canvas.width, h = canvas.height;

  // new: get sliders
  const time = document.getElementById('time3');
  const tVal  = document.getElementById('tVal3');
